...
export function SkillGraph({ skills, onSelect }: Props) {
  ...
  useEffect(() => {
    ...
    const links = skills.filter(s => s.parentSkillId).map(s => ({ source: s.parentSkillId!, target: s.id }))
    const sim = d3.forceSimulation<SimNode>(nodes)
      .force('link', d3.forceLink<SimNode, { source: string; target: string }>(links).id(d => d.id).distance(160))
  ...
  return (
    ...
    >
      {links.map(s => {
        ...
        return <line key={`l-${s.id}`} x1={from.x} y1={from.y} x2={to.x} y2={to.y} stroke="#4b5563" strokeWidth={2} />
      })}
      {skills.map(s => {
